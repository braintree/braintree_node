...
class WebhookNotification extends AttributeSetter {
  ...
  constructor(attributes, gateway) {
    ...
    if (wrapperNode.grantedPaymentInstrumentUpdate != null) {
      ...
      // NEXT_MAJOR_VERSION this was originally a typo, but since some merchants
      // may be relying on it, we include it as an alias for the correct property name.
      // remove this in the next major version release
      this.grantedPaymetnInstrumentUpdate = this.grantedPaymentInstrumentUpdate;
    }

    if (attributes.kind === Kind.GrantedPaymentMethodRevoked) {
