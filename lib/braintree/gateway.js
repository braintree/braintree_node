...
class Gateway {
  ...
  createSearchResponse(url, search, pagingFunction, callback) {
    ...
    gateway.http.post(url, {search: search.toHash()}, function (err, response) {
      if (err != null) {
        ...
        searchResponse.setFatalError(exceptions.ServiceUnavailableError('Service Unavailable')); // eslint-disable-line new-cap
      }
  ...
  searchResponseHandler(pagingFunction, callback) {
    ...
    return function (err, response) {
      ...
      return callback(exceptions.ServiceUnavailableError('Service Unavailable'), null); // eslint-disable-line new-cap
    };
  }
  ...
  pagingFunctionGenerator(search, url, SubjectType, pagedResultsKey, getSubject) {
    return (ids, callback) => {
      ...
      gateway.http.post(`${this.config.baseMerchantPath()}/` + url + '/advanced_search', {search: search.toHash()}, function (err, response) {
        if (err) {
          callback(err, null);
        ...
        callback(exceptions.ServiceUnavailableError('Service Unavailable'), null); // eslint-disable-line new-cap
      });
    };
  }
}
