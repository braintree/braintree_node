...
class Gateway {
  ...
  pagingFunctionGenerator(search, url, SubjectType, pagedResultsKey, getSubject, options) {
    return (ids, callback) => {
      let gateway = this.gateway;
      ...
      advancedSearch.ids().in(ids);
      gateway.http.post(`${this.config.baseMerchantPath()}/` + url + '/advanced_search', {search: advancedSearch.toHash()}, function (err, response) {
        if (err) {
          callback(err, null);
          return;
