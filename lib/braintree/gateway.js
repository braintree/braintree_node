...
class Gateway {
  ...
  pagingFunctionGenerator(search, url, SubjectType, pagedResultsKey, getSubject) {
    return (ids, callback) => {
      let gateway = this.gateway;
      let idsSearch = new TransactionSearch();

      idsSearch.ids().in(ids);
      gateway.http.post(`${this.config.baseMerchantPath()}/` + url + '/advanced_search', {search: idsSearch.toHash()}, function (err, response) {
        if (err) {
          callback(err, null);
          return;
