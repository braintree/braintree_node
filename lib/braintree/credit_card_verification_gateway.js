...
class CreditCardVerificationGateway extends Gateway {
  ...
  find(creditCardVerificationId) {
    if (Util.isInvalidPathSegment(creditCardVerificationId)) {
  ...
  search(fn, callback) {
    ...
    search.criteria.verification_type = ["credit_card"]; // eslint-disable-line camelcase
  ...
  pagingFunctionGenerator(search) {
    return (ids, callback) => {
      ...
      searchCriteria.verification_type = ["credit_card"]; // eslint-disable-line camelcase
