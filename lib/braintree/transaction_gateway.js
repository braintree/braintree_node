...
class TransactionGateway extends Gateway {
  ...
  search(fn, callback) {
    ...
    return this.createSearchResponse(`${this.config.baseMerchantPath()}/transactions/advanced_search_ids`, search, this.pagingFunctionGenerator(search, options), callback);
  }
  ...
  pagingFunctionGenerator(search, options) {
    let advancedSearch = search;

    if (options && options.idsOnlyPagination === true) {
      advancedSearch = new TransactionSearch();
    }

    return super.pagingFunctionGenerator(advancedSearch, 'transactions', Transaction, 'creditCardTransactions', response => response.creditCardTransactions.transaction, options);
  }
