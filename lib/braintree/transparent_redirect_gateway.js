var Digest = require('./digest').Digest,
    ...
    InvalidTransparentRedirectHashError = require('./exceptions/invalid_transparent_redirect_hash_error').InvalidTransparentRedirectHashError;
...
var TransparentRedirectGateway = function(gateway) {
  ...
  var confirm = function (queryString, callback) {
    // validate http status before query string

    if (!validateQueryString(queryString)) {
      callback(InvalidTransparentRedirectHashError(), null);
      return;
    };
