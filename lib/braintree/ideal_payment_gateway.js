...
class IdealPaymentGateway extends Gateway {
  ...
  find(idealPaymentId, callback) {
    if (idealPaymentId.trim() === '') {
      callback(exceptions.NotFoundError('Not Found'), null); // eslint-disable-line new-cap
      return;
    }

    this.gateway.http.get(`${this.config.baseMerchantPath()}/ideal_payments/${idealPaymentId}`, function (err, response) {
      if (err) {
        callback(err, null);
        return;
      }

      callback(null, new IdealPayment(response.idealPayment));
    });
  }

  sale(idealPaymentId, transactionRequest, callback) {
