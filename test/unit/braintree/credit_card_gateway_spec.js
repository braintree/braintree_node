...
describe("CreditCardGateway - path traversal", () => {
  const traversalIds = [
  ...
  describe("delete", () => {
    traversalIds.forEach((badId) => {
      it(`rejects token ${JSON.stringify(badId)} without calling http`, () => {
        return assertNotFoundAndNoHttp(gateway.delete(badId), httpStubs.delete);
      });
    });
  });
  ...
  describe("fromNonce", () => {
    traversalIds.forEach((badId) => {
      it(`rejects nonce ${JSON.stringify(badId)} without calling http`, () => {
        return assertNotFoundAndNoHttp(gateway.fromNonce(badId), httpStubs.get);
      });
    });
  });
