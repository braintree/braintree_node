...
let ExchangeRateQuoteGateway = require('../../../lib/braintree/exchange_rate_quote_gateway').ExchangeRateQuoteGateway;
...
describe('ExchangeRateQuoteGateway', () =>
  describe('generate', function () {
    ...
    it('does not accept invalid key', function (done) {
      ...
      exchangeRateQuoteGateway.generate(exchangeRateQuoteRequest, (err, params) => {
        assert.exists(err);
        assert.equal(err.name, 'invalidKeysError');
        assert.equal(err.type, 'invalidKeysError');
        assert.equal(err.message, 'These keys are invalid: quotes[invalidKey]');
