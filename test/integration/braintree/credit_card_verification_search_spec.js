  ...
  describe("search", function () {
      ...
      specHelper.defaultGateway.customer.create(customerParams, () =>
        specHelper.defaultGateway.creditCardVerification.search(
          function (search) {
            ...
            return search
              .status()
              .is(CreditCardVerification.StatusType.Verified);
          },
          ...
          function (err, response) {
            ...
            return response.first(function (err, verification) {
              ...
              assert.equal(
                verification.creditCard.expirationMonth,
                expirationMonth
              );
              assert.equal(
                verification.creditCard.expirationYear,
                expirationYear
              );
              assert.equal(
                verification.creditCard.cardType,
                CreditCard.CardType.Visa
              );
              assert.equal(
                verification.status,
                CreditCardVerification.StatusType.Verified
              );
    ...
    it("allows stream style interation of results", function (done) {
      ...
      specHelper.defaultGateway.customer.create(customerParams, function () {
        let search = specHelper.defaultGateway.creditCardVerification.search(
            ...
            function (err, response) {
              ...
              return response.each(function (err, verification) {
                ...
                if (verifications.length === 2) {
                  ...
                  assert.equal(
                    verifications[0].creditCard.cardholderName,
                    name
                  );
                  assert.equal(
                    verifications[1].creditCard.cardholderName,
                    name
                  );
          ...
          specHelper.defaultGateway.creditCardVerification.search(
            ...
            (err, response) =>
              response.first(function (err, verification) {
                ...
                assert.equal(verification.creditCard.cardholderName, name);
                assert.equal(
                  verification.creditCard.prepaid,
                ...
                assert.equal(
                  verification.creditCard.business,
                  CreditCard.Business.Unknown
                );
                assert.equal(
                  verification.creditCard.consumer,
                  CreditCard.Consumer.Unknown
                );
                assert.equal(
                  verification.creditCard.corporate,
                  CreditCard.Corporate.Unknown
                );
                assert.equal(
                  verification.creditCard.purchase,
                ...
                assert.equal(
                  verification.creditCard.commercial,
                  CreditCard.Commercial.Unknown
                );
                assert.equal(
                  verification.creditCard.healthcare,
                  CreditCard.Healthcare.Unknown
                );
                assert.equal(
                  verification.creditCard.debit,
                  CreditCard.Debit.Unknown
                );
                assert.equal(
                  verification.creditCard.payroll,
                  CreditCard.Payroll.Unknown
                );
                assert.equal(
                  verification.creditCard.countryOfIssuance,
                  CreditCard.CountryOfIssuance.Unknown
                );
                assert.equal(
                  verification.creditCard.issuingBank,
                  CreditCard.IssuingBank.Unknown
                );
                assert.equal(
                  verification.creditCard.productId,
