...
describe("DisputeSearch", () => {
  describe("callback", () => {
    it("returns no results", (done) => {
      specHelper.defaultGateway.dispute.search(
        (search) => {
    ...
    it("returns disputes by received date range", (done) => {
      specHelper.defaultGateway.dispute.search(
        function (search) {
          return search.receivedDate().between("03/03/2014", "03/05/2014");
        },
        function (err, response) {
          assert.isNull(err);
    ...
    it("returns disputes by disbursement date range", (done) => {
      specHelper.defaultGateway.dispute.search(
        function (search) {
          return search.disbursementDate().between("03/03/2014", "03/05/2014");
        },
        function (err, response) {
          assert.isNull(err);
          assert(response.length >= 1);
    ...
    it("returns disputes by effective date range", (done) => {
      specHelper.defaultGateway.dispute.search(
        function (search) {
          return search.effectiveDate().between("03/03/2014", "03/05/2014");
        },
        function (err, response) {
          assert.isNull(err);
          assert(response.length >= 1);
  ...
  describe("streams", () => {
    ...
    it("returns disputes by multiple reasons", (done) => {
      let stream = specHelper.defaultGateway.dispute.search(function (search) {
        return search
          .reason()
          .in([Dispute.Reason.ProductUnsatisfactory, Dispute.Reason.Retrieval]);
      });
      ...
      specHelper.defaultGateway.customer.create(
        ...
        (err, result) => {
          specHelper.defaultGateway.transaction.sale(
            ...
            (err, result) => {
              let stream = specHelper.defaultGateway.dispute.search(function (
                search
              ) {
                return search.customerId().is(result.transaction.customer.id);
              });
