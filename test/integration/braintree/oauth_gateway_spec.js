    ...
    it("returns validation errors when using a bad grant code", function (done) {
      ...
      return gateway.oauth.createTokenFromCode(
        ...
        function (err, response) {
          ...
          assert.equal(response.message, "Invalid grant: code not found");
    ...
    it("creates an access token from a refresh token", function (done) {
      ...
      specHelper.createGrant(
        ...
        (err, code) =>
          gateway.oauth.createTokenFromCode(
            { code, scope: "read_write" },
            (err, refreshTokenResponse) =>
              gateway.oauth.createTokenFromRefreshToken(
                {
                  refreshToken: refreshTokenResponse.credentials.refreshToken,
                  scope: "read_write",
                },
                function (err, response) {
                  assert.isNull(err);
    ...
    it("revokes an access token", function (done) {
      ...
      specHelper.createGrant(
        ...
        (err, code) =>
          gateway.oauth.createTokenFromCode(
            { code, scope: "read_write" },
            (err, accessTokenResponse) =>
              gateway.oauth.revokeAccessToken(
                accessTokenResponse.credentials.accessToken,
                function (err, response) {
                  assert.isNull(err);
                  assert.isTrue(response.success);
  ...
  describe("connectUrl", function () {
    ...
    it("encodes connect url query parameters containing special characters not encoded by encodeURIComponent", function () {
      ...
      let parts = Array.from(
        queryString
          .split("&")
          .find((item) => item.indexOf("wacky") > -1)
          .split("=")
      );
      let key = parts[0];
      let value = parts[1];
