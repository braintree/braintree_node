...
describe("DisputeGateway", () => {
  ...
  function createEvidenceDocument() {
    ...
    return specHelper.defaultGateway.documentUpload
      .create(documentUploadParams)
      .then((result) => {
  ...
  describe("self.accept", () => {
    ...
    it("returns error when dispute not open", () => {
      return disputeGateway.accept("wells_dispute").then((response) => {
        let error = response.errors.for("dispute").on("status")[0];

        assert.isFalse(response.success);
        assert.equal(
          ValidationErrorCodes.Dispute.CanOnlyAcceptOpenDispute,
          error.code
        );
        assert.equal(
  ...
  describe("self.addFileEvidence", () => {
    ...
    it("adds file evidence with category", () => {
      ...
      return createSampleDispute()
        ...
        .then((document) => {
          return disputeGateway.addFileEvidence(disputeId, {
    ...
    it("raises error when dispute not found", () => {
      return disputeGateway
        .addFileEvidence("unknown_dispute_id", "unknown_document_id")
        .catch((err) => {
          assert.equal("notFoundError", err.type);
          assert.equal(
            "dispute with id 'unknown_dispute_id' not found",
            err.message
          );
        });
    });
    ...
    it("raises error when dispute not open", () => {
      ...
      return createSampleDispute()
        ...
        .then((response) => {
          ...
          assert.isFalse(response.success);
          assert.equal(
            ValidationErrorCodes.Dispute.CanOnlyAddEvidenceToOpenDispute,
            error.code
          );
          assert.equal(
    ...
    it("raises error when category is not supported", () => {
      ...
      return createSampleDispute()
        ...
        .then((response) => {
          ...
          assert.isFalse(response.success);
          assert.equal(
            ValidationErrorCodes.Dispute.CanOnlyCreateEvidenceWithValidCategory,
            error.code
          );
        });
    });
    ...
    it("returns error when creating file evidence for text-only category", () => {
      ...
      return createSampleDispute()
        ...
        .then((response) => {
          ...
          assert.isFalse(response.success);
          assert.equal(
            ValidationErrorCodes.Dispute.EvidenceCategoryTextOnly,
            error.code
          );
        });
    });
  ...
  describe("self.addTextEvidence", () => {
    ...
    it("throws error when dispute not found", () => {
      return disputeGateway
        .addTextEvidence("unknown_dispute_id", "text evidence")
        .then(assert.fail)
        .catch((err) => {
          assert.equal(err.type, "notFoundError");
          assert.equal(
    ...
    it("returns error when creating text evidence for file-only category", () => {
      ...
      return createSampleDispute()
        ...
        .then((response) => {
          ...
          assert.isFalse(response.success);
          assert.equal(
            ValidationErrorCodes.Dispute.EvidenceCategoryDocumentOnly,
            error.code
          );
        });
    });
    ...
    it("returns error when creating text evidence with invalid date time format", () => {
      ...
      return createSampleDispute()
        ...
        .then((response) => {
          ...
          assert.isFalse(response.success);
          assert.equal(
            ValidationErrorCodes.Dispute.EvidenceContentDateInvalid,
            error.code
          );
        });
    });
  ...
  describe("self.finalize", () => {
    ...
    it("returns error when dispute not open", () => {
      return disputeGateway.finalize("wells_dispute").then((response) => {
        let error = response.errors.for("dispute").on("status")[0];

        assert.isFalse(response.success);
        assert.equal(
          ValidationErrorCodes.Dispute.CanOnlyFinalizeOpenDispute,
          error.code
        );
        assert.equal(
    ...
    it("returns error with digital goods missing", () => {
      ...
      return createSampleDispute()
        ...
        .then((response) => {
          let errorCodes = response.errors
            .for("dispute")
            .on("dispute")
            .map((e) => e.code);
          ...
          assert.include(
            errorCodes,
            ValidationErrorCodes.Dispute.DigitalGoodsMissingDownloadDate
          );
          assert.include(
            errorCodes,
            ValidationErrorCodes.Dispute.DigitalGoodsMissingEvidence
          );
        });
    });
    ...
    it("returns error with partial non-disputed transaction information", () => {
      ...
      return createSampleDispute()
        ...
        .then((response) => {
          ...
          assert.include(
            errorCodes,
            ValidationErrorCodes.Dispute
              .NonDisputedPriorTransactionEvidenceMissingDate
          );
        });
    });
  });
  ...
  describe("self.removeEvidence", () => {
    ...
    it("throws error when dispute or evidence not found", () => {
      return disputeGateway
        .removeEvidence("unknown_dispute_id", "unknown_evidence_id")
        .then(() => {
        ...
        .catch((err) => {
          ...
          assert.equal(
            err.message,
            "evidence with id 'unknown_evidence_id' for dispute with id 'unknown_dispute_id' not found"
          );
        });
    });
    ...
    it("returns error when dispute not open", () => {
      ...
      return createSampleDispute()
        ...
        .then((response) => {
          ...
          assert.isFalse(response.success);
          assert.equal(
            ValidationErrorCodes.Dispute.CanOnlyRemoveEvidenceFromOpenDispute,
            error.code
          );
          assert.equal(
