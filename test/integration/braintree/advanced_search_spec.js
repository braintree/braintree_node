...
describe("AdvancedSearch", function () {
  describe("textFields", function () {
    ...
    it("accepts the 'is' operator", (done) =>
      specHelper.defaultGateway.subscription.search(
        (search) => search.id().is(subscription1.id),
        function (err, response) {
    ...
    it("accepts the 'isNot' operator", (done) =>
      specHelper.defaultGateway.subscription.search(
        (search) => search.id().isNot(subscription1.id),
        function (err, response) {
          assert.isTrue(response.success);
    ...
    it("accepts the 'startsWith' operator", (done) =>
      specHelper.defaultGateway.subscription.search(
        (search) =>
          search
            .id()
            .startsWith(subscription1.id.slice(0, subscription1.id.length - 1)),
        function (err, response) {
          assert.isTrue(response.success);
          assert.equal(response.length(), 1);
    ...
    it("accepts the 'endsWith' operator", (done) =>
      specHelper.defaultGateway.subscription.search(
        (search) => search.id().endsWith(subscription1.id.slice(1)),
        function (err, response) {
          assert.isTrue(response.success);
          assert.equal(response.length(), 1);
    ...
    it("accepts the 'contains' operator", (done) =>
      specHelper.defaultGateway.subscription.search(
        (search) =>
          search
            .id()
            .contains(subscription1.id.slice(1, subscription1.id.length - 1)),
        function (err, response) {
          assert.isTrue(response.success);
          assert.equal(response.length(), 1);
  ...
  describe("multipleValueFields", function () {
    ...
    it("accepts the 'in' operator", (done) =>
      specHelper.defaultGateway.subscription.search(
        (search) => search.ids().in(subscription1.id, subscription2.id),
        function (err, response) {
          assert.isTrue(response.success);
    ...
    it("accepts the 'is' operator", (done) =>
      specHelper.defaultGateway.subscription.search(
        (search) => search.ids().is(subscription1.id),
        function (err, response) {
          assert.isTrue(response.success);
  ...
  describe("multipleValueOrTextFields", function () {
    ...
    it("accepts the 'is' operator", (done) =>
      specHelper.defaultGateway.subscription.search(
        (search) => search.planId().is(subscription1.planId),
        function (err, response) {
          assert.isTrue(response.success);
          assert.include(response.ids, subscription1.id);
          specHelper.doesNotInclude(response.ids, subscription2.id);
    ...
    it("accepts the 'isNot' operator", (done) =>
      specHelper.defaultGateway.subscription.search(
        (search) => search.planId().isNot(subscription1.planId),
        function (err, response) {
          assert.isTrue(response.success);
          specHelper.doesNotInclude(response.ids, subscription1.id);
          assert.include(response.ids, subscription2.id);
      ...
      specHelper.defaultGateway.subscription.search(
        (search) =>
          search
            .planId()
            .startsWith(
              subscription1.planId.slice(0, subscription1.planId.length - 1)
            ),
        function (err, response) {
          assert.isTrue(response.success);
          assert.include(response.ids, subscription1.id);
    ...
    it("accepts the 'endsWith' operator", (done) =>
      specHelper.defaultGateway.subscription.search(
        (search) => search.planId().endsWith(subscription1.planId.slice(1)),
        function (err, response) {
          assert.isTrue(response.success);
          assert.include(response.ids, subscription1.id);
          specHelper.doesNotInclude(response.ids, subscription2.id);
      ...
      specHelper.defaultGateway.subscription.search(
        (search) =>
          search
            .planId()
            .contains(
              subscription1.planId.slice(1, subscription1.planId.length - 1)
            ),
        function (err, response) {
          assert.isTrue(response.success);
          assert.include(response.ids, subscription1.id);
  ...
  describe("rangeFields", function () {
    ...
    it("accepts the 'min' operator", (done) =>
      specHelper.defaultGateway.subscription.search(
        (search) => search.price().min(Number(subscription1.price)),
        function (err, response) {
          assert.isTrue(response.success);
          assert.include(response.ids, subscription1.id);
          specHelper.doesNotInclude(response.ids, subscription2.id);
    ...
    it("accepts the 'max' operator", (done) =>
      specHelper.defaultGateway.subscription.search(
        (search) => search.price().max(Number(subscription2.price)),
        function (err, response) {
          assert.isTrue(response.success);
          assert.include(response.ids, subscription2.id);
          specHelper.doesNotInclude(response.ids, subscription1.id);
    ...
    it("accepts the 'between' operator", function (done) {
      ...
      specHelper.defaultGateway.subscription.search(
        (search) =>
          search
            .price()
            .between(subscriptionPrice - 0.01, subscriptionPrice + 0.01),
        function (err, response) {
          assert.isTrue(response.success);
          assert.include(response.ids, subscription1.id);
          specHelper.doesNotInclude(response.ids, subscription2.id);
