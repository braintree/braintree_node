  ...
  describe("find", function () {
          ...
          function (nonce) {
            ...
            specHelper.defaultGateway.paymentMethod.create(
              ...
              function (err, response) {
                ...
                specHelper.defaultGateway.usBankAccount.find(
                  usBankAccountToken,
                  function (err, usBankAccount) {
                    assert.isNull(err);
                    assert.equal(usBankAccount.last4, "1234");
                    assert.equal(
                      usBankAccount.accountHolderName,
                      "Dan Schulman"
                    );
                    ...
                    assert.isTrue(
                      usBankAccount.achMandate.acceptedAt instanceof Date
                    );
                    ...
                    assert.equal(
                      ...
                      UsBankAccountVerification.StatusType.IndependentCheck
                    );
    ...
    it("does not find invalid US bank account", (done) =>
      specHelper.defaultGateway.customer.create({}, function (err, response) {
        ...
        specHelper.defaultGateway.usBankAccount.find(
          specHelper.generateInvalidUsBankAccountNonce(),
          function (err, usBankAccount) {
          ...
          function (nonce) {
            ...
            specHelper.defaultGateway.paymentMethod.create(
              ...
              function (err, response) {
                ...
                specHelper.defaultGateway.usBankAccount.sale(
                  usBankAccountToken,
                  transactionParams,
                  function (err, response) {
                    assert.isTrue(response.success);
                    ...
                    assert.equal(
                      response.transaction.usBankAccount.last4,
                      "1234"
                    );
                    assert.equal(
                      response.transaction.usBankAccount.accountHolderName,
                      "Dan Schulman"
                    );
                    assert.equal(
                      response.transaction.usBankAccount.routingNumber,
                      "021000021"
                    );
                    assert.equal(
                      response.transaction.usBankAccount.accountType,
                      "checking"
                    );
                    assert.match(
