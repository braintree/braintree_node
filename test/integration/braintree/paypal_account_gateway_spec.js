...
describe("PayPalGateway", function () {
          ...
          function (err, response) {
            ...
            specHelper.defaultGateway.paypalAccount.find(
              ...
              function (err, paypalAccount) {
                ...
                assert.isNotNull(paypalAccount.fundingSourceDescription);
                assert.isNotNull(paypalAccount.editPaypalVaultId);
                done();
              }
            );
          }
                  ...
                  function (err, response) {
                    ...
                    specHelper.defaultGateway.paypalAccount.find(
                      ...
                      function (err, paypalAccount) {
                        ...
                        let subscriptionIds = [
                          paypalAccount.subscriptions[0].id,
                          paypalAccount.subscriptions[1].id,
                        ];
  ...
  describe("update", function () {
    ...
    beforeEach(function (done) {
      paymentMethodToken = Math.floor(Math.random() * Math.pow(36, 3)).toString(
        36
      );
    ...
    it("updates the paypal account", function (done) {
      let updateParams = { token: paymentMethodToken + "123" };

      specHelper.defaultGateway.paypalAccount.update(
        paymentMethodToken,
        updateParams,
        function (err, response) {
          assert.isNull(err);
          assert.isTrue(response.success);
          assert.equal(
            response.paypalAccount.token,
            paymentMethodToken + "123"
          );
        ...
        function (err, response) {
          ...
          specHelper.defaultGateway.paymentMethod.create(
            ...
            function (err, response) {
              ...
              let updateParams = { token: originalToken };
  ...
  describe("delete", function () {
    ...
    it("deletes the paypal account", (done) =>
      specHelper.defaultGateway.paypalAccount.delete(
        paymentMethodToken,
        function (err) {
          assert.isNull(err);
