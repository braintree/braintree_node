  ...
  describe("test settlement methods", function () {
    it("settles a transaction", function (done) {
      ...
      specHelper.defaultGateway.transaction.sale(
        transactionParams,
        function (err, transactionResponse) {
          assert.isNull(err);
    ...
    it("marks a transaction settlement pending", function (done) {
      ...
      specHelper.defaultGateway.transaction.sale(
        ...
        function (err, transactionResponse) {
          ...
          specHelper.defaultGateway.testing.settlementPending(
            transactionResponse.transaction.id,
            function (err, settleResponse) {
              assert.isNull(err);
              assert.equal(
                settleResponse.transaction.status,
                "settlement_pending"
              );
    ...
    it("marks a transaction settlement confirmed", function (done) {
      ...
      specHelper.defaultGateway.transaction.sale(
        ...
        function (err, transactionResponse) {
          ...
          specHelper.defaultGateway.testing.settlementConfirm(
            transactionResponse.transaction.id,
            function (err, settleResponse) {
              assert.isNull(err);
              assert.equal(
    ...
    it("marks a transaction settlement declined", function (done) {
      ...
      specHelper.defaultGateway.transaction.sale(
        ...
        function (err, transactionResponse) {
          ...
          specHelper.defaultGateway.testing.settlementDecline(
            transactionResponse.transaction.id,
            function (err, settleResponse) {
              assert.isNull(err);
              assert.equal(
    ...
    it("throws an error if testing gateway settlement methods are used in production", function (done) {
      ...
      gateway.testing.settlementConfirm("transaction_id", function (err) {
        assert.equal(
          err.type,
          braintree.errorTypes.testOperationPerformedInProductionError
        );
