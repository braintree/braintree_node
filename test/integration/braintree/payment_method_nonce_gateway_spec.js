...
describe("PaymentMethodNonceGateway", function () {
  ...
  describe("create", function () {
    it("creates the nonce", (done) =>
      specHelper.defaultGateway.paymentMethodNonce.create(
        paymentMethodToken,
        function (err, response) {
    ...
    describe("regulationEnvironment", function () {
      it("can return unregulated", function (done) {
        let params = getPaymentMethodNonceParams(EUROPEAN_MERCHANT_TOKEN, {
          amount: AMOUNT_THRESHOLD_FOR_RBI,
        });

        specHelper.defaultGateway.paymentMethodNonce.create(
          INDIAN_PAYMENT_TOKEN,
          params,
          function (err, response) {
            let authenticationInsight =
              response.paymentMethodNonce.authenticationInsight;

            assert.equal(
              "unregulated",
              authenticationInsight.regulationEnvironment
            );
            done();
          }
        );
    ...
    describe("scaIndicator", function () {
      ...
      it("can return sca_required with amount over the threshold", function (done) {
        let params = getPaymentMethodNonceParams(INDIAN_MERCHANT_TOKEN, {
          amount: AMOUNT_THRESHOLD_FOR_RBI + 1,
        });
      ...
      it("can return sca_optional with amount within threshold", function (done) {
        let params = getPaymentMethodNonceParams(INDIAN_MERCHANT_TOKEN, {
          amount: AMOUNT_THRESHOLD_FOR_RBI,
          recurringCustomerConsent: true,
          recurringMaxAmount: AMOUNT_THRESHOLD_FOR_RBI,
        });
  ...
  describe("find", function () {
    it("find the nonce", function (done) {
      ...
      specHelper.defaultGateway.paymentMethodNonce.find(
        nonce,
        function (err, paymentMethodNonce) {
          assert.isNull(err);
