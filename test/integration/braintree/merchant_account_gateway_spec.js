...
let MerchantAccount =
  require("../../../lib/braintree/merchant_account").MerchantAccount;
...
describe("MerchantAccountGateway", function () {
  describe("create", function () {
    it("accepts the legacy parameters", (done) =>
      specHelper.defaultGateway.merchantAccount.create(
        legacyMerchantAccountParams,
        function (err, response) {
          assert.isNull(err);
          assert.isTrue(response.success);
          assert.equal(
            response.merchantAccount.status,
            MerchantAccount.Status.Pending
          );
          assert.equal(
            response.merchantAccount.masterMerchantAccount.id,
    ...
    it("allows an id to be passed", function (done) {
      ...
      specHelper.defaultGateway.merchantAccount.create(
        ...
        function (err, response) {
          ...
          assert.equal(
            response.merchantAccount.id,
            `sub_merchant_account_id${rand}`
          );
      ...
      specHelper.defaultGateway.merchantAccount.create(
        ...
        function (err, response) {
          ...
          assert.equal(
            response.errors
              .for("merchantAccount")
              .on("masterMerchantAccountId")[0].code,
            ValidationErrorCodes.MerchantAccount
              .MasterMerchantAccountIdIsRequired
          );
    ...
    describe("funding destination", function () {
      ...
      it("accepts a mobile phone", function (done) {
        ...
        params.funding.destination =
          MerchantAccount.FundingDestination.MobilePhone;
  ...
  describe("update", function () {
    it("updates the Merchant Account info", function (done) {
      ...
      specHelper.defaultGateway.merchantAccount.update(
        ...
        function (err, response) {
          ...
          assert.equal(
            response.merchantAccount.individual.email,
            "john.doe@example.com"
          );
          assert.equal(
            response.merchantAccount.individual.dateOfBirth,
            "1970-01-01"
          );
          ...
          assert.equal(
            response.merchantAccount.individual.address.streetAddress,
            "123 Fake St"
          );
          assert.equal(
            response.merchantAccount.individual.address.locality,
            "Chicago"
          );
          assert.equal(
            response.merchantAccount.individual.address.region,
            "IL"
          );
          assert.equal(
            response.merchantAccount.individual.address.postalCode,
            "60622"
          );
          assert.equal(
            response.merchantAccount.business.dbaName,
            "James's Bloggs"
          );
          assert.equal(
            response.merchantAccount.business.legalName,
            "James's Bloggs Inc"
          );
          assert.equal(response.merchantAccount.business.taxId, "423456789");
          assert.equal(
            response.merchantAccount.business.address.streetAddress,
            "999 Fake St"
          );
          assert.equal(
            response.merchantAccount.business.address.locality,
            "Miami"
          );
          assert.equal(response.merchantAccount.business.address.region, "FL");
          assert.equal(
            response.merchantAccount.business.address.postalCode,
            "99999"
          );
          assert.equal(
            response.merchantAccount.funding.accountNumberLast4,
            "8798"
          );
          assert.equal(
            response.merchantAccount.funding.routingNumber,
            "071000013"
          );
          assert.equal(
            response.merchantAccount.funding.email,
            "check@this.com"
          );
          assert.equal(
            response.merchantAccount.funding.mobilePhone,
            "1234567890"
          );
          assert.equal(
            response.merchantAccount.funding.destination,
            MerchantAccount.FundingDestination.MobilePhone
          );
          assert.equal(
            response.merchantAccount.funding.descriptor,
            "James Bloggs FL"
          );
        ...
        function (err, response) {
          ...
          assert.equal(
            response.errors
              .for("merchantAccount")
              .for("individual")
              .on("dateOfBirth")[0].code,
            ValidationErrorCodes.MerchantAccount.Individual
          ...
          assert.equal(
            response.errors
              ...
              .for("individual")
              .for("address")
              .on("streetAddress")[0].code,
            ValidationErrorCodes.MerchantAccount.Individual.Address
              .StreetAddressIsRequired
          );
          assert.equal(
            response.errors
              .for("merchantAccount")
        ...
        function (err, response) {
          ...
          assert.equal(
            response.errors
              ...
              .for("address")
              .on("streetAddress")[0].code,
            ValidationErrorCodes.MerchantAccount.Business.Address
              .StreetAddressIsInvalid
          );
          assert.equal(
            response.errors
          ...
          assert.equal(
            response.errors
              .for("merchantAccount")
              .for("funding")
              .on("accountNumber")[0].code,
            ValidationErrorCodes.MerchantAccount.Funding.AccountNumberIsInvalid
          );
          assert.equal(
            response.errors.for("merchantAccount").for("funding").on("email")[0]
              .code,
        ...
        function (err, response) {
          ...
          assert.equal(
            response.errors
              ...
              .for("business")
              .on("legalName")[0].code,
            ValidationErrorCodes.MerchantAccount.Business
              .LegalNameIsRequiredWithTaxId
          );
