...
let generate3DSNonce = function(params, callback) {
  let responseCallback = function(err, response) {
    ...
    return callback(threeDSecureNonce);
  };
...
let generateNonceForNewPaymentMethod = function(paymentMethodParams, customerId, callback) {
  let myHttp = new ClientApiHttp(new Config(specHelper.defaultConfig));
...
let createPayPalTransactionToRefund = function(callback) {
  ...
  return generateNonceForNewPaymentMethod(nonceParams, null, function(nonce) {
    ...
    return defaultGateway.transaction.sale(transactionParams, function(err, response) {
      ...
      return specHelper.settlePayPalTransaction(transactionId, (err, settleResult) =>
...
let createPlanForTests = (attributes, callback) =>
  specHelper.defaultGateway.http.post(`${defaultGateway.config.baseMerchantPath()}/plans/create_plan_for_tests`, {plan: attributes}, (err, resp) => callback())
;

let createModificationForTests = (attributes, callback) =>
  specHelper.defaultGateway.http.post(`${defaultGateway.config.baseMerchantPath()}/modifications/create_modification_for_tests`, {modification: attributes}, (err, resp) => callback())
;
